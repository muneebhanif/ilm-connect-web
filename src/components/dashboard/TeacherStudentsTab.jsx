          ...
          {studentsLoading || scheduleLoading ? (
            ...
            <div className="space-y-4">
              {todayAttendanceRows.map((row) => (
                <div key={row.key} className="rounded-[24px] border border-parchment/50 bg-gradient-to-r from-white to-ivory/70 p-5">
                  <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        <StatusPill tone={getAttendanceStatusTone(row.status)}>{getAttendanceStatusLabel(row.status)}</StatusPill>
                        <span className="text-xs font-semibold uppercase tracking-[0.18em] text-bark/70">{row.sessionLabel}</span>
                      </div>
                      <div className="mt-2 text-lg font-semibold text-ink">{row.studentName}</div>
                      <div className="mt-1 text-sm text-bark">{row.courseTitle}</div>
                      <div className="mt-1 text-xs text-bark">{row.durationMinutes ? `${row.durationMinutes} min class` : 'Use this row to keep today’s attendance ready.'}</div>
                    </div>
                    <div className="flex flex-wrap gap-2 lg:max-w-[320px] lg:justify-end">
                      ...
                      <button onClick={() => updateAttendanceByKey(row.attendanceKey, 'unmarked')} className="rounded-xl border border-parchment bg-white px-3 py-2 text-sm font-semibold text-ink-soft hover:border-rose/20 hover:text-rose">Reset</button>
                    </div>
                  </div>
                </div>
              ))}
